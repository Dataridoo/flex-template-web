...
import { searchListings, searchMapListings, setActiveListing } from './EventsSearchPage.duck';
import {
...
export class EventsSearchPageComponent extends Component {
  constructor(props) {
    super(props);
  ...
  // too often (in the middle of map's pan or zoom activity)
   onIdle(googleMap) {
    ...
    if (viewportBoundsChanged && !this.modalOpenedBoundsChange) {
      ...
      history.push(
        createResourceLocatorString('EventsSearchPage', routeConfiguration(), {}, searchParams)
      );
    } else {
      this.viewportBounds = viewportBounds;
      this.modalOpenedBoundsChange = false;
...
EventsSearchPageComponent.defaultProps = {
  listings: [],
  mapListings: [],
  pagination: null,
...
EventsSearchPageComponent.propTypes = {
  listings: array,
  mapListings: array,
...
const EventsSearchPage = compose(withRouter, connect(mapStateToProps, mapDispatchToProps), injectIntl)(
  EventsSearchPageComponent
);

EventsSearchPage.loadData = (params, search) => {
